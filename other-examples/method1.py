"""
AuthLX Python SDK – Integration Method 1
==========================================
**Method 1: Your code sits BELOW the login call.**

This is the recommended approach.  The AuthLX login gate is the very first
thing that runs.  If login succeeds, execution falls through naturally into
your application logic.  There is no way to call your protected functions
without first passing the authentication check.

Usage
-----
1.  pip install requests
2.  Set APP_ID, APP_NAME, and APP_VERSION below.
3.  Add your application logic inside `run_app()`.
4.  python method1.py

Why Method 1?
-------------
- Simplest to understand and audit.
- Authentication happens at the top of the call stack before any application
  code is reachable.
- Cannot be bypassed by calling your functions out of order (unlike Method 2).
"""
...
# ============================================================
#  ★  METHOD 1: Your application code starts here
#
#  The login() call acts as a gate.  Everything inside
#  run_app() is only reachable after successful authentication.
# ============================================================
...
def run_app(authlxapp: api):
    """
    Protected application logic – only called after a successful login.
    ...
    print("  [+] Protected feature 1 running…")
    print("  [+] Protected feature 2 running…")
    ...
    while True:
        ...
        if choice == "1":
            ...
            if authlxapp.login(user, password):
                run_app(authlxapp)   # ← your code runs ONLY if login passes
            # ─────────────────────────────────────────────────────────────
            else:
                print("  ✗ Login failed.")
